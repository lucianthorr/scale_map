...
void check_switches()
{
  ...
  BUTTONvar1 = shiftIn(BUTTONdataPin, BUTTONclockPin);
  BUTTONvar2 = shiftIn(BUTTONdataPin, BUTTONclockPin);
  BUTTONvar3 = shiftIn(BUTTONdataPin, BUTTONclockPin);
  BUTTONvar4 = shiftIn(BUTTONdataPin, BUTTONclockPin);
  BUTTONvar5 = shiftIn(BUTTONdataPin, BUTTONclockPin);

  //write the button states to the currentstate array
  for (int i = 0; i<=7; i++)
  {
    buttons[i] = BUTTONvar1 & (1 << i);
  }
  for (int i = 8; i<=15; i++)
  {
    buttons[i] = BUTTONvar2 & (1 << (i%8));
  }
  for (int i = 16; i<=23; i++)
  {
    buttons[i] = BUTTONvar3 & (1 << (i%8));
  }
  for (int i = 24; i<=31; i++)
  {
    buttons[i] = BUTTONvar4 & (1 << (i%8));
  }
  for (int i = 32; i<=39; i++)
  {
    buttons[i] = BUTTONvar5 & (1 << (i%8));
  }
  ...
  for (index = 0; index < 40; index++) {
    ...
    //currentstate[index] = buttons[index];   // read the button state from the array
